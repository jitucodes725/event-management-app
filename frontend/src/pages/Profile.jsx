...
function Profile() {
  ...
  const [profileData, setProfileData] = useState({ name: '', bio: '', phoneNumber: '' });
    ...
    API.get('/auth/profile')
      .then((res) => {
        ...
        if (res.data.profilePic) {
          setPicPreview(`http://localhost:5000${res.data.profilePic}`);
        }
      })
  ...
  const handleProfileSave = async (e) => {
    ...
    if (Object.keys(v).length) return setProfileErrors(v);
  ...
  const inputClass = (field, errorsObj = pwErrors) =>
    `w-full px-4 py-3 rounded-xl border ${
      errorsObj[field] ? 'border-red-400' : 'border-gray-200 dark:border-gray-700'
          ...
          <form onSubmit={handleProfileSave} className="flex flex-col gap-4">
            ...
            <div>
              <input
                ...
                maxLength={10}
                inputMode="numeric"
                className={inputClass('phoneNumber', profileErrors)}
              />
